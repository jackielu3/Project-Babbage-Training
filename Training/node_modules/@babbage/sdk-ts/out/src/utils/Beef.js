...
class BeefTx {
    ...
    get txid() {
        ...
        if (this._rawTx)
            return this._txid = asString(doubleSha256BE(Buffer.from(this._rawTx)));
...
class Beef {
     ...
     * @param allowKnown optional. If true, transaction txid is assumed valid
     */
    async verify(chainTracker, allowKnown) {
    ...
    verifyValid(allowKnown, chainTracker) {
    ...
    static fromReader(br) {
        ...
        if (version !== exports.BEEF_MAGIC)
            throw new Error(`Serialized BEEF must start with ${exports.BEEF_MAGIC} but starts with ${version}`);
        const beef = new Beef();
